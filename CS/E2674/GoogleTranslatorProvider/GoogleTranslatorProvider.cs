...
namespace GoogleTranslatorProvider {
    public class GoogleTranslatorProvider : TranslatorProviderBase {
        private static string[] GoogleLanguages = 
            ...
            while (index < text.Length) {
                ...
                if (leftSeparatorIndex >= 0) {
                    ...
                    if (rightSeparatorIndex >= 0) {
                        string block = text.Substring(index, leftSeparatorIndex - index).Trim();
                        if (block.Length > 0) {
                            result.Add(block);
                        }
        ...
        protected string TranslateCore(string text, string sourceLanguageCode, string desinationLanguageCode) {
        ...
        internal static string DecodeASCIIToUnicode(string text) {
            ...
            while ((unicodeSymbolIndex = result.IndexOf("\\u")) >= 0) {
                ...
                string unicodeChar = char.ConvertFromUtf32(Convert.ToInt32(unicodeCharStringCode, 16));
        ...
        public override IEnumerable<string> CalculateSentences(string text) {
            ...
            IEnumerable<string> lineBlocks = text.Split(new string[] { Environment.NewLine, "\\r\\n", "\\n" }, StringSplitOptions.RemoveEmptyEntries);
